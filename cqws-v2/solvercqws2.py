...
#         self.naLHBinding = inputfile.naLHBinding
    ...
    def create_structure_arrays(self):
        ...
        EgAlAs   = lambda T: 2.766 - (6e-4*T**2/(408+T))
...
def Schrodinger(model,sparse = False,absolute = False):
    ...
    Hamiltonian_lh = H(-poth,mlh,dx,n)
    Hamiltonian_hh = H(-poth,mhh,dx,n)
    meV = 1e3
...
class Solver:
    ...
    def QuantumSolutions(self,absolute = False,Print = False):
            ...
            self.EHH   = np.zeros(self.subbands)
            self.ELH   = np.zeros(self.subbands)
            self.Hamiltonian_e  = H(self.pote,self.me,self.dx,self.n)
            self.Hamiltonian_lh = Hholes(self.poth,self.mlh,self.dx,self.n)
            self.Hamiltonian_hh = Hholes(self.poth,self.mhh,self.dx,self.n)
            meV = 1e3
...
#                     self.Psihh[:,i] = np.power(np.absolute(self.wfe_hh[:,i]),2)
            ...
            for i in range(self.subbands):
                if i%2 == 0:
                    self.DirectTransitions.append(["E%d->HH%d :" %(i,i), " %.4f"%(self.EHH[i])])
                    self.DirectTransitions.append(["E%d->LH%d :" %(i,i), " %.4f"%(self.ELH[i])])
                else:
                     self.DirectTransitions.append(["E%d->HH%d :" %(i,i), " %.4f"%(self.EHH[i])])
                     self.DirectTransitions.append(["E%d->LH%d :" %(i,i), " %.4f"%(self.ELH[i])])
            ...
            if Print == True:
                ...
                print(tabulate(self.eprinte,['ELECTRON [eV]','HEAVY HOLE [eV]','LIGHT HOLE [eV]'],
                              tablefmt='orgtbl',stralign='center',floatfmt='.5f'))
    ...
    def print_result(self,structure_name,results):
        ...
        trans_file.write('\nTHIS PROGRAM CALCULATE OPTICAL TRANSITIONS OF SAMPLE: '+structure_name+'\n\n')
        trans_file.write(tabulate([["Paramters"],
                                    ["Temperature:         %.1f"%self.T],
                                    ["Ban Offset Ratio:    %.2f:%.2f"%(self.Qc,self.Qv)],
    ...
    def plotting(self,results,amp=10,axmin=50,axmax=50,eymin = 0,eymax=0,hymin=0,hymax=0):
            ...
            ax3.plot(self.xaxis/nm,self.VB,ls='-',lw='2',color='gray')
            
            for i in range(self.subbands):
                ax3.plot(self.xaxis/nm,amp*self.WF_hh[:,i]-self.Ehh[i],
                         ls='-',
                         lw='2',
                         color=colors[i],
                         label = '$\psi hh_%d$'%(i))
            ...
            ax1.set_ylabel(r'$\mathrm{Conduction\,\, Band\,\, [eV]}$',fontsize=10)
            ax1.yaxis.set_label_coords(-0.14,0.5)
            ax1.set_xlim(xmin,xmax)  
            ax1.set_ylim(eymin,eymax)
            # outliers only
            ax3.legend(loc = 0,fontsize=10)
            ax3.set_ylim(hymin,hymax)  # most of the data
            ax3.set_ylabel(r'$\mathrm{Valence\,\, Band\,\, [eV]}$',fontsize=10)
            ax3.set_xlabel(r'$\mathrm{Growth\,\, Direction\,\, [nm]}$',fontsize=10)
            ...
            ax3.plot((0, +d), (1 , 1 + 0), **kwargs)  # bottom-left diagonal
            ax3.plot((1 - d, 1 + 0), (1 - 0, 1 + 0), **kwargs)  # bottom-right diagonal
            
            
            
            
            
            # Plot electron and light holes
            
            
            ax2.plot(self.xaxis/nm,self.CB,ls='-',lw='2',color='gray')
            
            for i in range(self.subbands):
                ax2.plot(self.xaxis/nm,amp*self.WF_e[:,i]+self.Ee[i],
                        ls='-',
                        lw='2',
                        color=colors[i],
                        label = '$\psi e_%d$'%(i))
            ...
            ax4.plot(self.xaxis/nm,self.VB,ls='-',lw='2',color='gray')
            
            for i in range(self.subbands):
                ax4.plot(self.xaxis/nm,amp*self.WF_lh[:,i]-self.Elh[i],
                         ls='-',
                         lw='2',
                         color=colors[i],
                         label = '$\psi lh_%d$'%(i))
            ...
            ax2.set_ylabel(r'$\mathrm{Conduction\, Band\, [eV]}$',fontsize=10)
            ax2.yaxis.set_label_coords(-0.14,0.5)
            ax2.set_xlim(xmin,xmax)  
            ax2.set_ylim(eymin,eymax)
            # outliers only
            ax4.legend(loc = 0,fontsize=10)
            ax4.set_ylim(hymin,hymax)  # most of the data
            ax4.set_ylabel(r'$\mathrm{Valence\, Band\, [eV]}$',fontsize=10)
            ax4.set_xlabel(r'$\mathrm{Growth\, Direction\, [nm]}$',fontsize=10)
            ...
            # how big to make the diagonal lines in axes coordinates
            # arguments to pass to plot, just so we don't keep repeating them
            kwargs = dict(transform=ax2.transAxes, lw=1,color='k', clip_on=False)
            ax2.plot((0, +d), (0, 0), **kwargs)        # top-left diagonal
            ax2.plot((l - d, 0 + l), (0, +0), **kwargs)


            kwargs.update(transform=ax4.transAxes)  # switch to the bottom axes
            ax4.plot((0, +d), (1 , 1 + 0), **kwargs)  # bottom-left diagonal
            ax4.plot((1 - d, 1 + 0), (1 - 0, 1 + 0), **kwargs)  # bottom-right diagonal
            


            # What's cool about this is that now if we vary the distance between
            f.subplots_adjust(hspace=0.075,wspace=0.3) #or plt.subplot_tool(),
